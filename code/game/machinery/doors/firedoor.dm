...
/obj/machinery/door/firedoor/proc/unregister_adjacent_turfs(atom/old_loc)
	...
	UnregisterSignal(our_turf, COMSIG_TURF_LIQUIDS_CHANGE) // SPLURT EDIT ADDITION
	for(var/dir in GLOB.cardinals)
		var/turf/checked_turf = get_step(our_turf, dir)
		...
		UnregisterSignal(checked_turf, COMSIG_TURF_LIQUIDS_CHANGE) // SPLURT EDIT ADDITION
...
/obj/machinery/door/firedoor/proc/process_results(datum/source)
	...
	if(!(checked_turf.flags_1 & INITIALIZED_1)) // uninitialized turfs won't have atmos setup anyways, so check_atmos would just complain and not work
		return
	var/result = water_sensor ? (check_atmos(checked_turf) || check_liquids(checked_turf)) : check_atmos(checked_turf) // SPLURT EDIT, ORIGINAL "var/result = check_atmos(checked_turf)"
