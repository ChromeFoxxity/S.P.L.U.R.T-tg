...
/datum/action/innate/clockcult/add_warp_area
	...
	var/static/list/blocked_areas = typecacheof(list(/area/station/service/chapel, /area/station/ai/satellite))
...
/datum/action/innate/clockcult/add_warp_area/proc/build_addable_areas()
	...
	for(var/area/station_area as anything in GLOB.the_station_areas)
		...
		if(station_area.outdoors || (station_area.area_flags & NOTELEPORT) || is_type_in_typecache(station_area, blocked_areas) || (station_area in GLOB.clock_warp_areas))
			continue
